...
from assistant import get_answer, evaluate
from db import (
    save_conversation,
    save_feedback,
...
def main():
    ...
    model_choice = st.selectbox(
        "Select a model:",
        ["ollama/phi3", "openai/gpt-3.5-turbo", "openai/gpt-4o",
    ...
    if st.button("Ask"):
        ...
        with st.spinner("Processing..."):
            ...
            ans_cost = answer_data['cost']
            if ans_cost > 0:
                st.write(f"Answer cost: ${ans_cost:.4f}")
        with st.spinner("Processing evaluation..."):
            start_time = time.time()
            eval_data = evaluate(user_input, answer_data)
            end_time = time.time()
            print_log(f"Evaluation received in {end_time - start_time:.2f} seconds")
            # Display monitoring information
            st.write(f"Relevance: {eval_data['relevance']}")
            st.write(f"Response time: {eval_data['response_time']:.2f} seconds")
            st.write(f"Evaluator Model used: {eval_data['model_used']}")
            st.write(f"Evaluator Total tokens: {eval_data['total_tokens']}")
            eval_cost = eval_data['cost']
            if eval_cost > 0:
                st.write(f"Evaluator cost: ${eval_cost:.4f}")
            total_cost = ans_cost + eval_cost
            if total_cost > 0:
